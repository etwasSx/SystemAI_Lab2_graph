...
class MinimizingTotalScore(ABCClass):  # В этом алгоритме мы изначально должны знать самый короткий путь от начальной вершины ко всем другим, а нужен этот алгоритм для того, чтобы восстановить этот самый самый короткий путь
    pre_calc: List
    ...
    def find_shortest_path(self, start, end):
        ...
        while True:
            ...
            for city_idx, distance in self.adj_matrix[node]:  # В этом цикле мы находим вершину, докоторой расстояние от начальной самоей минимальное
    ...
    def pre_calculate_dist(self, start_idx):  # Тут мы находим кратчайшее расстояние до всех вершин из текущих с помощью bfs
        k = deque()
        k.append(start_idx)
        while len(k) != 0:
            node = k.popleft()
