...
class FirstBestMatch(ABCClass):
    # Алгоритм максимально тупой, мы просто на каждом шаге выбираем ребро с минимальным весом,
    # если мы зашли в тупик, то просто возвращаемся назад по рекурсии и выбираем второе по величине ребро
    used: List
    path: List = []
    flag: bool = False
    # Он нужен для того, чтобы остановить рекурсию, когда мы дошли до нашей конечной точки(end_idx)
    end_idx: int
    ...
    def first_best_match_dfs(self, node):
        # В целом, алгоритм очень похож на классический dfs, только теперь мы переходим в вершины по увеличению расстояния до них
        # (т.е. сначала перейдём в вершину, до которой самое короткое ребро из текущей вершины и т.д.)
        if self.flag:
            return
